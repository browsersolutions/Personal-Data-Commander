...
const URI_plugin_user_read_all_agreements =
  "/plugin_user_read_all_data_agreements";
const URI_plugin_user_delete_data_agreement =
  "/plugin_user_delete_data_agreement";
const URI_plugin_user_read_data_agreement = "/plugin_user_read_data_agreement";
const URI_plugin_user_set_agreement_active_status =
  "/plugin_user_set_agreement_active_status";
const URI_plugin_user_deactivate_agreements =
  "/plugin_user_deactivate_agreements";
const URI_plugin_user_activate_agreements = "/plugin_user_activate_agreements";
...
async function fetchData() {
  try {
    ...
    // Fetch data from web service (replace with your actual API endpoint)
    const response = await fetch(
      server_url + URI_plugin_user_read_all_agreements,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    data.forEach((row) => {
      ...
      suspendActButton.addEventListener("change", async (e) => {
        if (e.target.checked) {
          await suspendByUUID(row.uuid);
        } else {
          await activateByUUID(row.uuid);
        }
      });
      // // Add suspend button
...
document
  .getElementById("dataAgreementsRefreshButton")
  .addEventListener("click", fetchData);

document
  .getElementById("dataAgreementsActivateAllButton")
...
document
  .getElementById("dataAgreementsSuspendAllButton")
